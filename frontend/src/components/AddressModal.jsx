...
const AddressModal = ({ isOpen, onClose, onAddressAdded }) => {
  ...
  const handleSelectSaved = (e) => {
    ...
    const addr = savedAddresses.find(
      (a) => a.address_id.toString() === selectedId,
    );
    if (addr) {
      setFormData({
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 p-4 backdrop-blur-md">
      <div className="bg-(--second-text-color) w-full max-w-md p-8 flex flex-col gap-4 relative border border-(--main-text-color)">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-xl text-(--main-text-color) hover:text-(--hover-color) transition-all"
        >
          ✕
        </button>

        <h2 className="text-2xl font-semibold uppercase tracking-tighter text-(--main-text-color)">
          Shipping Address
        </h2>
        ...
        {savedAddresses.length > 0 && (
          <div className="flex flex-col gap-1 mb-2">
            <label className="text-[10px] uppercase tracking-widest text-(--main-text-color) opacity-60">
              Use saved address
            </label>
            <select
            ...
            <div className="flex items-center gap-2 my-2">
              ...
              <span className="text-[10px] uppercase text-(--main-text-color) opacity-40">
                Or enter new
              </span>
              <hr className="flex-1 border-(--main-text-color) opacity-20" />
            </div>
