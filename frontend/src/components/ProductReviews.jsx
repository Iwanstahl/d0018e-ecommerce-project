...
const ProductReviews = ({ productData, onRefresh }) => {
  ...
  const loadComments = async () => {
    if (productData?.product_id) {
      const data = await ratingService.getProductComments(
  ...
  return (
    <div className="pt-10">
      ...
      <div className="flex flex-col md:flex-row gap-12 bg-(--main-text-color) p-8 text-(--second-text-color)">
        ...
        <div className="flex flex-col items-center justify-center md:border-r border-(--second-text-color)/10 md:pr-12">
          ...
          <div className="flex gap-1 my-3 text-yellow-400">
            {[...Array(5)].map((_, i) => (
              <span key={i}>
                {i < Math.round(productData.average_rating) ? "★" : "☆"}
              </span>
            ))}
          </div>
          <p
            ...
            >
              ...
              <div className="flex-1 h-1.5 bg-(--second-text-color)/10 overflow-hidden">
                <div
                  ...
                  style={{
                    width: getProgressWidth(productData.ratings?.[score] || 0),
                  }}
                ></div>
              </div>
              <span className="w-4 opacity-50">
                {productData.ratings?.[score] || 0}
              </span>
            </div>
          ))}
        </div>
          ...
          <div className="flex gap-2">
            {[1, 2, 3, 4, 5].map((star) => (
              ...
              >
                <span
                  className={
                    star <= hoverScore ? "text-yellow-400" : "text-white/20"
                  }
                >
                  ★
                </span>
      ...
      <div
        ref={commentsRef}
        className="space-y-6 pt-6 border-t border-(--main-text-color)"
      >
        <h4 className="text-xs font-bold uppercase tracking-widest mb-6 text-(--main-text-color)">
          Customer Comments
        </h4>
        ...
        {comments.length > 0 ? (
          comments
            .filter((r) => r.comment)
            .map((item, index) => (
              <div key={index} className="bg-(--main-text-color) p-4 mb-2">
                ...
                <p className="text-sm opacity-75 leading-relaxed text-(--second-text-color) ">
                  "{item.comment}"
                </p>
          ...
          <p className="text-xs opacity-40 text-(--main-text-color)">
